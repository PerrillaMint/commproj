...
void OperatorConsole::HandleConsoleInputs() {
    ...
    std::cout << "  4. settime <seconds> - Change collision prediction time constraint (n)\n";
    ...
    while (!exit) {
        ...
        // Spec: the controller must be able to change the parameter n at runtime
        else if (command == "settime") {
            int newTime;

            if (iss >> newTime) {
                if (newTime <= 0) {
                    std::cerr << "Time constraint must be a positive integer (seconds)\n";
                    continue;
                }

                // Send CHANGE_TIME_CONSTRAINT_COLLISIONS message to ComputerSystem
                // This is handled locally by ComputerSystem via its processMessage()
                // We send it through the Comms channel which can forward it
                int comms_channel = name_open(COMMS_CHANNEL_NAME, 0);
                ...
                msg.planeID = -1;  // Not plane-specific

                // Store the new time value in the data field
                int timeVal = newTime;
                msg.dataSize = sizeof(int);
                std::memcpy(msg.data.data(), &timeVal, sizeof(int));
                ...
                std::cerr << "Invalid settime command format. Usage: settime <seconds>\n";
            }
        }
        else {
