...
void Radar::writeToSharedMemory() {
	...
	*/
    ...
    std::vector<msg_plane_info>& bufferToUse = activeBuffer.empty() ? inactiveBuffer : activeBuffer;
    ...
    if (bufferToUse.empty()) {
        ...
        shared_mem->count = bufferToUse.size();
        std::copy(bufferToUse.begin(), bufferToUse.end(), shared_mem->plane_data);
        bufferToUse.clear();
    }
