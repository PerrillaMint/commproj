...
//Coen320_Lab3 (Task0): Radar Channel name should contain your group name
#define Radar "AH_UA_40281796_40252823_Radar" //attach point for AirTrafficControl
...
Aircraft::Aircraft(int id, double x, double y, double z, double sx, double sy, double sz, int t)
    ...
    if (pthread_create(&thread_id, NULL, updatePositionThread, (void*)this) != 0) {
        throw std::runtime_error("Failed to create thread for aircraft " + std::to_string(id));
    }
}
...
void Aircraft::changeHeading(double Vx, double Vy, double Vz) {
	//issue found and fixed: checked Vx three times if it was bigger than 0
    if (Vx != 0) speedX = Vx;
    if (Vy != 0) speedY = Vy;
    if (Vz != 0) speedZ = Vz;

    //show changed velocity
...
int Aircraft::updatePosition() {
    ...
    while (true) {
        ...
        if (posX < airspace.lower_x_boundary || posX > airspace.upper_x_boundary ||
            ...
            break; // Send exit airspace message and exit loop if out of bounds
        }
        ...
        if (rcvid != -1) {
        	 // Determine message type by checking the MessageType field and recas
