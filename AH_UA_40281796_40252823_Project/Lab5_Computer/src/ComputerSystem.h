...
public:
    ...
    // Spec: the controller must be able to change parameter n at runtime
    // Static atomic so CommunicationsSystem can update it from the same process
    static std::atomic<int> timeConstraintCollisionFreq;
...
private:
    ...
    // Returns -1 = no collision, 0 = current collision, 1 = predicted collision (warning)
    int checkAxes(msg_plane_info plane1, msg_plane_info plane2);
