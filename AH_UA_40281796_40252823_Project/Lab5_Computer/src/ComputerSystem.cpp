...
bool ComputerSystem::initializeSharedMemory() {
    ...
    while (true) {
    	...
    	shared_mem = (SharedMemory*)mmap(NULL, shm_size, PROT_READ, MAP_SHARED, shm_fd, 0);
...
void ComputerSystem::checkCollision(uint64_t currentTime, std::vector<msg_plane_info> planes) {
	...
	for (size_t i = 0; i < planes.size(); i++) {
	    for (size_t j = i + 1; j < planes.size(); j++) {
	        const msg_plane_info& p1 = planes[i];
	        const msg_plane_info& p2 = planes[j];
	        int result = checkAxes(p1, p2);
	        CollisionSeverity severity = (result == 0) ? CollisionSeverity::COLLISION : CollisionSeverity::WARNING;
	        if (result >= 0) {
	            collisionPairs.push_back({p1.id, p2.id, severity});
	        }
	    }
	}
    // COEN320 Task 3.5
    ...
    if (!collisionPairs.empty()) {
        size_t dataSize = collisionPairs.size() * sizeof(CollisionPair);
